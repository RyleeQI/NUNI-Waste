...
"""
train:
python maskrcnn/train_net.py --config-file maskrcnn/configs/*config*.yaml (optional, only use if trained on TACO-zerowaste) 
    --taco --dataroot /path/to/zerowaste/data/ (optional) 
    --resume OUTPUT_DIR /maskrcnn/outputs/*experiment_name* (optional) 
    --MODEL.WEIGHTS /path/to/checkpoint.pth

eval:
python maskrcnn/train_net.py --config-file deeplab/configs/*config*.yaml (optional, only use if evaluated on TACO-zerowaste) 
    --taco 
    --dataroot /path/to/zerowaste-or-taco/data/  
    --eval-only OUTPUT_DIR /maskrcnn/outputs/*ex
"""
...
def register_zero_waste_instances(data_root): 
    ...
    register_coco_instances("zero-waste-train", {}, 
                            data_paths["train"][1], 
                            data_paths["train"][0])
    register_coco_instances("zero-waste-val", {}, 
                            data_paths["val"][1], 
                            data_paths["val"][0])
    register_coco_instances("zero-waste-test", {}, 
                            data_paths["test"][1], 
                            data_paths["test"][0])    
...
def register_taco_instances(data_root): 
    ...
    for split in ["train", "test"]:
        ...
        ann_path = opj(data_root, split, "labels.json")
        data_paths[split] = (img_folder, ann_path)
    register_coco_instances("taco-zw-train", {}, 
                            data_paths["train"][1], 
                            data_paths["train"][0])
    register_coco_instances("taco-zw-test", {}, 
                            data_paths["test"][1], 
                            data_paths["test"][0])
...
class Trainer(DefaultTrainer):
    ...
    def build_test_loader(cls, cfg, dataset_name):
        mapper = DatasetMapper(cfg, is_train=False, 
                               augmentations=[T.Resize((1024, 2048)),])
        return build_detection_test_loader(cfg, 
                                           dataset_name=dataset_name, mapper=mapper)
...
# Removed 55 images with no usable annotations. 2947 images left. 3002
if __name__ == "__main__":
    parser = default_argument_parser()
    parser.add_argument("--dataroot", type=str, 
                        default="/root/autodl-tmp/zerowaste_database_resized/",
                         help="root folder for the dataset on the disk")
    parser.add_argument("--taco", type=bool, 
                        default=True,
