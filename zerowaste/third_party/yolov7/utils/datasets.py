...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadZeroWasteImagesAndLabels(path, imgsz, batch_size,
    ...
    dataloader = loader(dataset,
                        ...
                        sampler=sampler,
                        pin_memory=True,
                        collate_fn=LoadZeroWasteImagesAndLabels.collate_fn4 if quad else LoadZeroWasteImagesAndLabels.collate_fn)
...
class LoadZeroWasteImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, 
                 path, 
                 img_size=640, 
                 batch_size=16, 
        ...
        self.label_files = glob.glob(os.path.join(self.path, "yolo_labels", "*.txt"))
        valid_files_prefix = [os.path.basename(label_file).split(".")[0] for 
                              label_file in self.label_files]
        img_files = glob.glob(os.path.join(self.path, "data", "*.PNG"))
        self.img_files = []
        for img_file in img_files:
            if os.path.basename(img_file).split(".")[0] in valid_files_prefix:
                self.img_files.append(img_file)
        # Check cache
        p = Path(img_files[0])  # os-agnostic
